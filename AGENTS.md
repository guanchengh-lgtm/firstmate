...
5. **Prior-session fold** - after the read-once contract, `bin/fm-prior-session-fold.sh` prints only a bounded fold of live jobs, durable keyed open picks, non-actionable unverified pick context, and captain words that changed a lock from this home's prior Pi or Claude JSONL, or a loud incomplete result when that source cannot support the fold.
