...
The supervision branch is default-on (docs/pi-supervision-branch.md): whenever this session owns the fleet lock and away mode is not active, the watcher extension hands wholly eligible task-local queues and selected fleet-wide heartbeat reviews to the persistent in-process supervision branch.
Any queue with a main-only row during an eligibility check remains in this conversation.
The offer remains pending until the branch stores an outcome through `fm_branch_report`; a turn without that report returns the wake to this conversation, the third consecutive unreported turn stops the branch loop, and a successful report resets the streak.
The branch reuses its generation-bound session below its context budget and rotates to a fresh persistent session above that budget without rewriting the old session file.
