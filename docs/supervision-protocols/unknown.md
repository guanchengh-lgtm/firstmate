...
Ordinary wake: drain, handle every item required by the harness-neutral drain contract in [`AGENTS.md`](../../AGENTS.md#8-supervision-protocol), and run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`, then repeat that verified wait while supervision is still required.
