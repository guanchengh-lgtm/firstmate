...
The guard covers the main primary and genuinely marked secondmate homes, exempts child crewmate/scout worktrees from supervision scope, is loop-safe per harness, and is documented in [turnend-guard.md](turnend-guard.md); that owner names the spec compile-check Stop adapter as the child-worktree exception.
