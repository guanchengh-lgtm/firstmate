...
[`bin/fm-record.sh`](../bin/fm-record.sh) is the only code that commits, fetches, fast-forwards, pushes, or proves equality for the Record; its `reconcile`, `checkpoint --reason maintain`, `tick`, and `verify` seams are the whole write surface the night task uses, so no second transaction framework, scheduler, or Git policy exists.
