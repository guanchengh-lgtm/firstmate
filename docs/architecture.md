...
The guard covers the main primary and genuinely marked secondmate homes, exempts child crewmate/scout worktrees from supervision scope, and is documented in [turnend-guard.md](turnend-guard.md); that owner names per-harness loop safety and the spec compile-check Stop adapter as the child-worktree exception.
