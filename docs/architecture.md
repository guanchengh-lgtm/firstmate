...
`bin/fm-nm-run-lib.sh` owns the exact run-head identity rules.
...
A ship spawn also requires explicit `--role builder|verifier`; `bin/fm-brief.sh` and `bin/fm-spawn.sh` own machine sibling markers under `data/<id>/` for mode and role (including the no-mistakes verifier second context) and refuse a missing or mismatched marker rather than scanning brief prose, so the worker's launch inputs and the recorded task delivery cannot diverge.
...
[`bin/fm-validation-truth-lib.sh`](../bin/fm-validation-truth-lib.sh) refuses no-mistakes arm, merge, and non-force cleanup unless a fresh `bin/fm-crew-state.sh` read reports `source: run-step` and `state: done`; direct-PR, local-only, scout, and secondmate tasks are exempt, and an unreadable validation truth fails closed with that wording rather than restarting the shared no-mistakes daemon.
