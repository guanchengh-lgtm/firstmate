...
`bin/fm-primary-scope-lib.sh` owns the linked-worktree home refusal used by session start, lock, guard, watch, teardown, and both merge entrypoints.

## Event-driven supervision

A zero-token bash watcher (`bin/fm-watch.sh`) sleeps on the fleet, classifies detected wakes in bash, and wakes the first mate only when something is actionable.
The watcher excludes its own pane target through `bin/fm-supervisor-target-lib.sh`.
...
A main drain also prints `STATUS OUTCOME BACKSTOP` when the newest captain-facing task event has no covering branch outcome, even when no queue row remains.
[`pi-supervision-branch.md`](pi-supervision-branch.md#status-outcome-backstop) owns that bounded lost-wake backstop.
...
Each home atomically publishes that bounded home summary with freshness epoch metadata at `state/home-summary.json` after a locked session start, a watcher-observed status or turn-end signal, task spawn, task teardown, and on a recurring live-watcher cadence; `bin/fm-home-summary-refresh.sh` owns the publication mechanics.
...
The Bearings projection also adds the required prior-session retrieve from `bin/fm-prior-session-fold.sh`.
...
`fm-send.sh` is the data plane and refuses recorded-task lifecycle text or keys before any effect, preventing an exit command from becoming routing-marked chat that the agent reasons about instead of executing.
...
When the file exists, `fm-spawn.sh` refuses crewmate and scout launches without explicit harness, model, and effort axes, so `config/crew-harness` is only automatic when no dispatch profile file is active.
...
The header of `bin/fm-delivery-surface-lib.sh` owns the task-surface values and the direct-PR eligibility gate shared by those three scripts.
...
`bin/fm-brief.sh` writes the mode, builder role, and optional validated surface markers on a first ship scaffold.
`bin/fm-spawn.sh` records a validated surface in task metadata and revalidates the recorded value on relaunch; a legacy `direct-PR` record without `surface=internal-only` cannot relaunch.
`bin/fm-promote.sh` records the validated surface and `role=builder`, then writes the builder markers on a scout-to-ship flip so a later ship respawn can recover the delivery contract from metadata rather than from brief prose, and a verifier remains a later `--role verifier` spawn.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records validated PR metadata through `bin/fm-pr-check.sh` before calling the forge CLI.
The helper requires a full canonical URL and rejects malformed URLs or repo override flags before recording merge state.
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, pins `--match-head-commit` to the live forge head so GitHub enforces `expectedHeadOid`, refuses a red or pending check rollup, and follows the merge-method contract in [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh).
A GitLab merge request URL is refused before any merge state is recorded, while its watch and poll path stays supported; [docs/gitlab-merge-watch.md](gitlab-merge-watch.md) owns that limitation.
After the forge command returns, the script confirms the pull request actually landed, and only a confirmed landing records a landed outcome; a queued or unconfirmed request records none and leaves its poll armed.
An outcome that is neither merged nor queued is refused loudly and non-zero, naming the observed state, and a base branch that requires the merge queue is refused with the concrete retry flags its configured method requires rather than having a merge method chosen on the caller's behalf.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the complete task safety gate, the scout scratch exception, explicit task discard, and when leftover cleanup runs.
[`bin/fm-git-cleanup-lib.sh`](../bin/fm-git-cleanup-lib.sh) owns the shared committed-work proofs, leftover inventory, the keep-set, safe sibling removal, checked branch deletion, and report-only retention of gone registrations and orphan copies for teardown and fleet sync.
...
[`bin/fm-nightly.sh`](../bin/fm-nightly.sh) owns ordered execution, the single run lock under the Record's `.git/`, the transcript archive, and the LaunchAgent; [`bin/fm-maintain.py`](../bin/fm-maintain.py) owns every deterministic Record check, the report-then-enforce rollout, the generated views under `data/wiki/views/` except the gbrain trees owned by [`docs/gbrain.md`](gbrain.md), the T2 measures, and the digest reader that session start prints.
[`bin/fm-graphify.sh`](../bin/fm-graphify.sh) is the T10 compiler invocation that nightly calls after views; it does not add a second scheduler.
