...
`tests/fm-owner-invoke-wait-check.test.sh` covers the helper's fail-closed input, exact-count fixtures, held locked-next, owner-invoke yes-ask, fog-pin wait, required-skill omission, durable ship gather for skills and OV report, current-turn skill-load scoping, and invoked-skill acceptance.
