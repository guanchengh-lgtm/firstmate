...
The same family refuses a held locked next act with no worker, a tight `OWNER_INVOKE_WAIT` marker on an owner-invoke skill, and durable ship records that omit an OV report or never loaded plan-eng-review; `bin/fm-owner-invoke-wait-check.sh` owns those rules.
...
The owner-invoke helper's header owns held locked-next, the `OWNER_INVOKE_WAIT` marker, fog gather, missing OV report, unloaded plan-eng-review, fail-closed CLI input, and residual coverage.
...
`tests/fm-owner-invoke-wait-check.test.sh` covers the helper's fail-closed input, exact-count fixtures, held locked-next, owner-invoke yes-ask, fog-pin wait, required-skill omission, freeze-list negative control, and invoked-skill acceptance.
