...
The owner-invoke helper's header owns held locked-next, owner-node-open one-message-late completion, the header node table, session-scoped ship gather (`session=` matching `state/.lock`), the Stop ladder on `data/<ov>/report.md` then live OV *agent* (not bare pane husk) then refuse, fail-closed CLI input, and residual coverage.
Spawn-time brief checks retain the plan-eng-review skill rule, and Claude crewmate `settings.local.json` wires absolute-path PostToolUse Skill recording through `bin/fm-skill-load-record.sh`.
