...
The owner-invoke helper's header owns held locked-next, the `OWNER_INVOKE_WAIT` marker, fog gather, session-scoped ship gather (`session=` matching `state/.lock`), the Stop ladder on `data/<ov>/report.md` then live OV endpoint then refuse, report-gated `data/<ov>/skills` for plan-eng-review, fail-closed CLI input, and residual coverage.
...
`tests/fm-owner-invoke-wait-check.test.sh` covers the helper's fail-closed input, exact-count fixtures, held locked-next, owner-invoke yes-ask, fog-pin wait, session-scoped ship gather, live-worker-no-report pass, torn-down worker with report, report-gated skills refusal, skill-load recorder normalization, current-turn skill-load scoping, and invoked-skill acceptance.
