...
`/updatefirstmate` updates each remote code root from `upstream` when configured and otherwise `origin`, then guardedly fast-forwards the persistent remote home to that code-root commit.
