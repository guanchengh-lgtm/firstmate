...
`/updatefirstmate` updates each remote code root from the local source selected by [`config/update-remote`](configuration.md#self-update-remote-configupdate-remote), then guardedly fast-forwards the persistent remote home to that code-root commit.
