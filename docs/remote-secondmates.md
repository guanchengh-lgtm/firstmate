...
`/updatefirstmate` updates each remote code root using that remote home's [`config/update-remote`](configuration.md#self-update-remote-configupdate-remote) preference, then guardedly fast-forwards the persistent remote home to that code-root commit.
