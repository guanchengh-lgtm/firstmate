...
Session-lock ownership in `bin/fm-session-lock-lib.sh` resolves the contiguous harness ancestry first, then uses a valid Claude session id from the hook environment with `state/.lock.session`; all other harnesses and uncertain Claude states keep the ancestry comparison.
On 2026-08-30, a Claude Code 2.1.251 lab launch explicitly unset `CLAUDE_CODE_SESSION_ID`, `CLAUDE_PID`, and `CLAUDE_CODE_CHILD_SESSION`, then started `claude -p --session-id 15f2cb7b-0000-4000-8000-000000000002 --model haiku --effort low --settings <lab>/settings.json --setting-sources project --no-session-persistence`.
Its Stop hook ran `/usr/bin/env > <lab>/stop-env.txt` and recorded `CLAUDE_CODE_SESSION_ID=15f2cb7b-0000-4000-8000-000000000002`, `CLAUDE_PID=45008`, and `CLAUDE_CODE_CHILD_SESSION=1`, proving that the hook carried its session's own id.
