...
This setting is not inherited into secondmate homes because each home's remote topology is local to that home; a remote route therefore reads that remote home's file when selecting the host code-root fetch source.
...
The default conversation budget is 25% of the active model's context window, or 120000 tokens when no model window is known.
`FM_BRANCH_CONTEXT_BUDGET_TOKENS` accepts a positive decimal integer within JavaScript's safe integer range as an override; absent or invalid values use the default.
When the model window is known, the effective budget is capped at `max(1, contextWindow - 16384)` to preserve Pi's compaction reserve.
...
A standalone-clone home cannot receive a primary-local commit through that no-fetch sync, so it receives the rule through `/updatefirstmate`'s preferred-remote refresh instead.
...
When `config/crew-dispatch.json` exists, crewmate and scout spawns require explicit resolved harness, model, and effort axes instead of automatically falling back to `config/crew-harness`.
...
Session-start bootstrap is the only automatic caller of this check.
It runs in detect-only mode so findings surface without blocking session start or auto-editing captain material, while registry structural failures remain loud.
...
The same Stop adapter also runs [`bin/fm-reduce-check.sh`](../bin/fm-reduce-check.sh) on declared wide-work counts; residual limits live in [`turnend-guard.md`](turnend-guard.md#known-residual-gap).
[`bin/fm-spec-compile-stop-check.sh`](../bin/fm-spec-compile-stop-check.sh)'s header owns write detection, home derivation from the written path, the child-worktree seat before primary-scope, fail-open cases, and residual coverage.
Each matcher header owns its rules, structural empty input, and matcher limits.
The refuse is not a session-start diagnostic and does not read `FM_HOME` as an implicit compile or reduce home.
It ships in the tracked checkout and reaches a running home only after that home updates firstmate.
...
It prints one `MAP_FOG` line for each map with the finding count and a pointer to the checker for full details.
...
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit harness (`--harness`, a positional adapter, or a raw launch command), `--model`, or `--effort`.
`--model default` and `--effort default` name the selected harness's own default for that axis and still count as chosen.
Batch spawns satisfy the same requirement with shared `--harness`, `--model`, and `--effort`.
...
An omitted model or effort in the file means the selected harness uses its own default for that axis; the spawn still requires `--model` and `--effort`, and `default` is the token that records that choice.
...
While the file remains present, no crewmate or scout spawn may proceed without explicit resolved harness, model, and effort axes; malformed configuration must be reported and corrected rather than selected around.
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when direct axi status cannot be attributed
