...
Use `fm-remote-home-seed.sh <id> <ssh-alias> <remote-root> <remote-home> {<project>[=<origin-url>]...|--no-projects}` to provision a whole home on an SSH-reachable host, naming each project's origin so no local clone of it is needed.
