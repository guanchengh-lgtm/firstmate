...
| `fm-owner-invoke-wait-check.sh` | Refuse a locked next act, an owner-invoke node still open after the captain's next message with no matching artifact, or this session's ships whose OV review is gone without a report; spawn-time brief checks retain the Claude plan-eng-review load rule |
