...
| `fm-bearings-snapshot.sh` | Project the bounded remote-ledger fleet snapshot to compact TOON with the required prior-session retrieve; `--include-prs` adds live GitHub enrichment |
...
| `fm-wake-drain.sh`       | Present and acknowledge durable watcher wakes plus the script-owned recovery sections, then assert supervision health; its header owns the exact sections and cursor rules |
...
| `fm-pr-merge.sh`         | Record PR metadata, pin the forge head, refuse a red or pending rollup, merge a task's canonical full GitHub URL, then refuse an outcome it cannot prove landed or queued |
...
| `fm-teardown.sh`         | Fail-closed teardown: landed ship work, scout report presence and captain-hold completion, no-mistakes validation truth, secondmate retirement |
