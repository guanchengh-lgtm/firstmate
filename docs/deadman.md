...
The probe treats an ordinary `state/*.meta` file or `state/x-watch.check.sh` as live work, but a `kind=secondmate` record is a standing registration and not live work.
