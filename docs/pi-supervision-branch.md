...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates and reopens the persistent branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  After wake rows are claimed, a branch prompt counts as handled only when `fm_branch_report` appends a durable outcome; a later provider error cannot return the already stored outcome for duplicate handling.
  A prompt with no report releases the grant and rejects delivery ownership back to the watcher.
  A successful report resets `consecutiveUnreportedWakes`; the third consecutive unreported wake stops the serialized branch loop until session replacement.
  When that third failure is a settled provider error, the branch also surfaces a one-line health note and enters bounded recovery.
  ...
  A prompt from the current branch generation and model or effort selection that appends a durable `fm_branch_report` clears both the latch and unreported-wake streak and surfaces a one-line recovery note.
  A session replacement or branch model or effort change resets the recovery state and breaker immediately.
  After each turn, the branch compares context use with one quarter of the active model window, capped below Pi's compaction reserve.
  An over-budget session releases branch leases, is disposed, and clears only the current-session pointer; its append-only session file remains unchanged, and the next wake creates a fresh persistent session.
  `FM_BRANCH_CONTEXT_BUDGET_TOKENS` can override that threshold for an operator or focused test but cannot cross the reserve.
...
- Outcome store: `bin/fm-branch-outcome.sh`; its header owns the append-only format and read cursor.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is eligible only when every unread row is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below).
...
After the visible entry exists and the read cursor has passed it, the extension hands every still-unprocessed captain row to main as one hidden, typed `fm-branch-process` request (kind `branch-outcome`) listing each `[seq N] task: summary`.
...
The branch accepts it only when every unread row is a heartbeat row or a resolvable task-local signal or stale event.
A main-only row, an unresolvable task-local row, an unknown row kind, or an unreadable queue defers the whole review to main.
...
Cache reuse is best-effort and ends before context exhaustion.
...
The branch-offer, heartbeat-offer, mixed-queue fallback, and main-only-check-class tests remain in `tests/fm-pi-watch-extension.test.sh`, and the recovery test remains in `tests/fm-session-start.test.sh`.
