...
// captain-facing outcome opens a sequence-keyed processing request on main
...
//
// Failure direction: a wake completes only after fm_branch_report stores its
// outcome. Every other turn result rejects its settlement to the watcher,
// which retains delivery ownership and routes the wake to MAIN through its
// consumption-acknowledged path. The branch rotates before context exhaustion,
// and three consecutive unreported wakes stop its loop until replacement.
// A broken branch declines later offers, so they take that same watcher path.
...
//
// Model and effort selection: the captain can pin a different model and
// reasoning effort for the branch alone with /supervision-model, which picks from Pi's own catalog and
...
// A provider failure rejects immediately to watcher-owned fallback but leaves
// room for a transient outage to recover on the next wake. A third consecutive
// unreported wake latches the branch off. While provider-latched, main keeps every wake
...
type VisibleOutcomeRecord = OutcomeRecord & { version: 1 };
...
function parseVisibleOutcomeRecord(value: unknown): VisibleOutcomeRecord | null {
  ...
  const row = parseOutcomeRecord(value);
  return row ? { version: 1, ...row } : null;
}

function sameOutcome(left: OutcomeRecord, right: OutcomeRecord): boolean {
...
export default function (pi: ExtensionAPI) {
  ...
  function recordUnreportedWake(detail: string): void {
  ...
  function recordSettledProviderError(detail: string): void {
    consecutiveUnreportedWakes += 1;
    if (consecutiveUnreportedWakes < UNREPORTED_WAKE_BREAKER_THRESHOLD && !providerRecovery) return;
  ...
  // A reopened branch records its last effort, so an unpinned branch must
  // apply main's effort after a pin is cleared. Pi owns the clamp, so a level the branch's
  // model does not support becomes that model's nearest supported level
  // rather than a refusal. Only
  ...
  function rotateBranchIfOverBudget(branchForWake: BranchSession, expectedGeneration: number): void {
    const tokens = branchForWake.session.getContextUsage()?.tokens;
    if (typeof tokens !== "number" || tokens <= contextBudgetTokens(branchForWake.session)) return;
    if (
      !actingAsOwner(expectedGeneration) ||
      branch !== branchForWake ||
      branchForWake.generation !== expectedGeneration ||
  ...
  function ensureVisibleCaptainOutcome(row: OutcomeRecord): boolean {
  ...
  function deliverRoutineOutcome(row: OutcomeRecord): void {
    const message = {
  ...
  function readUnprocessedOutcomes(expectedGeneration: number): OutcomeRecord[] | null {
    ...
    for (const line of listed.stdout.split("\n")) {
      ...
      try {
        row = parseOutcomeRecord(JSON.parse(line));
      } catch {
        row = null;
      }
  ...
  function processingRequestInput(rows: OutcomeRecord[]): string {
...
${context.command}
  ...
  function enqueueWake(message: string, acceptedGeneration: number, recoveryProbe = false): Promise<void> {
    ...
    const delivery = branchChain
      .then(async () => {
        ...
        try {
          rotateBranchIfOverBudget(branchForWake, acceptedGeneration);
        } catch (error) {
          rotationError = error;
        }
        if (durableReportRevision > reportRevisionBeforePrompt) {
          ...
          if (rotationError && actingAsOwner(acceptedGeneration)) {
        ...
        const rotationDetail =
          rotationError instanceof Error
            ...
            : rotationError !== undefined
              ...
              : "";
        if (providerError) {
          const detail = `supervision branch provider failed after construction: ${providerError}${rotationDetail ? `; ${rotationDetail}` : ""}`;
        ...
        const detail =
          ...
          `${promptDetail ? `: ${promptDetail}` : ""}${rotationDetail ? `; ${rotationDetail}` : ""}`;
        if (actingAsOwner(acceptedGeneration) &&
          branchForWake.generation === generation &&
          branchForWake.selectionRevision === branchSelectionRevision
        ) {
          recordUnreportedWake(detail);
        }
        throw new Error(detail);
      })
      .catch((error: unknown) => {
  ...
  // Pi's ModelSelectorComponent is not reused because its selection
  // handler writes the captain's default model through Pi's settings manager,
  // which would change main's model while pinning the branch.
  // Ordering and filtering live in lib/fm-branch-model-picker.ts.
  async function pickBranchModel(
    ...
    const picked = await ctx.ui.custom<string | null>((tui, theme, keybindings, done) => {
      ...
      container.addChild(new Text(muted("type to search, arrow keys navigate, enter selects, esc cancels"), 1, 0));
      container.addChild(new DynamicBorder(accent));
