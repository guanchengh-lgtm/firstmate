...
You resolve each project's origin yourself - from the captain, the project registry, a clone that exists elsewhere, or `gh-axi` - and name it as `<project>=<origin-url>`; the seed validates and transports what you supply.
A remote seed therefore creates nothing in this home beyond the route, the charter, and its launch record: never clone a project into `projects/`, initialize no-mistakes here, or run a fleet sync just to seed a remote secondmate.
A bare `<project>` remains a convenience for a project this home already has cloned, whose configured origin is read instead.
[`bin/fm-project-origin-lib.sh`](../../../bin/fm-project-origin-lib.sh) owns the accepted origin forms; [`docs/remote-secondmates.md`](../../../docs/remote-secondmates.md) owns the rest of the operator contract.
