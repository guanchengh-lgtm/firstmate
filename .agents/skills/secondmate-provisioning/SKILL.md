...
This section is the single owner of the secondmate sync and inherited-local-material propagation contract; `AGENTS.md` sections 5 and 6 load this skill for secondmate recovery and home operations.
...
`/updatefirstmate` first updates the remote code root from `upstream` when configured and otherwise `origin`, then runs that guarded home sync.
