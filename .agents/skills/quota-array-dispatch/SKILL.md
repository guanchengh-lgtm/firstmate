...
5. When two comparable candidates both have supported evidence that they can finish through that horizon, apply reset bias from the applicable windows rather than from a single collapsed soonest bound.
   ...
   If no near spend-hard or two-to-three-day case applies and both candidates sit on a similar farther horizon, such as roughly four to six days, do not prefer the earlier reset.
   For that far-horizon remaining-quota comparison, compare each candidate's weekly / `seven_day` window `percentRemaining` directly; fall back to scope `effectivePercentRemaining` only when no such longer-cycle bound exists on that candidate.
   Never let a short rolling window such as `five_hour` supply the remaining value or invert the far-horizon choice, and never sort candidates by soonest bounding-window reset alone; a weekly-only peer must not lose a near spend-hard case merely because the other candidate also carries a longer cycle.
