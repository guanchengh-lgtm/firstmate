...
Never hand-type an interrupt key or exit command through `fm-send`: it refuses recorded-task lifecycle payloads and points to `fm-control`, preventing routing-marked lifecycle text from becoming chat the agent reasons about instead of executing ([`docs/agent-control.md`](../../../docs/agent-control.md)).
...
The primary integrations for `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `cursor` also have wired PreToolUse-equivalent hooks that deny watcher-arm anti-patterns and direct agent-shell `gh` execution before they run.
...
When changing this shell-command PreToolUse hook, validate the real harness behavior in a scratch project before trusting it, then update that doc.
...
| grok | `--model <model>` | `--reasoning-effort <low\|medium\|high\|xhigh>` | Verified on grok 1.0.5 (2026-08-22). `--effort` is an alias, but firstmate's profile axis is reasoning effort. As of 1.0.5 the accepted set is `xhigh, high, medium, low`; `max` is rejected with `use one of: xhigh, high, medium, low`, so firstmate omits `max`. |
...
[`docs/calm.md`](../../../docs/calm.md) owns Calm authority and worker scope across Pi launch contexts.
...
## grok (VERIFIED 2026-06-29, grok 0.2.73; slash-submit re-verified 2026-07-03 on 0.2.82; reasoning-effort ceiling re-verified 2026-08-22 on 1.0.5; exit paths re-verified 2026-07-19 on grok 0.2.103)
...
`fm-teardown` removes the task token and its registry entry; the pooled worktree's own pointer file is removed by `fm-spawn` when it sanitizes a freshly leased worktree, so a returned pool slot can never fire signals for a dead task.
