...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/t9-brief.5yvm/state/demo-ship.status'`
...
Firstmate steers you through durable message files in '/tmp/t9-brief.5yvm/state/demo-ship.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/t9-brief.5yvm/state/demo-ship.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/t9-brief.5yvm/state/demo-ship.inbox'/NNN.msg '/tmp/t9-brief.5yvm/state/demo-ship.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/Users/AI/.no-mistakes/worktrees/edb446952c22/01M2BT3QBT2SDW1NWQFY81ME2N/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md` that lacks `## Maintaining this file`, add that short self-governance section from `/Users/AI/.no-mistakes/worktrees/edb446952c22/01M2BT3QBT2SDW1NWQFY81ME2N/bin/fm-ensure-agents-md.sh` in the same pass.
