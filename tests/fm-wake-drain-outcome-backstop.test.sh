...
# S4 append does not write statusEndpoint or presentation indexes. Plant the
# cache U append would have left so same-second and junk-history cases stay
# independent of a later helper.
plant_task_outcome_index() {  # <state> <task> <seq>
  local state=$1 task=$2 seq=$3 f="$1/$2.status" size ident
  size=$(_fm_status_file_size "$f") || return 1
  size=${size//[[:space:]]/}
  ident=$(_fm_open_decisions_file_ident "$f") || return 1
  printf 'fm-branch-outcome-index-v1\t%s\t%s\t%s\n' "$seq" "$size" "$ident" \
    > "$state/.$task.branch-outcome-index" || return 1
  printf '%s\n' "$seq" > "$state/.branch-outcome-index-ready"
}
...
test_same_second_outcome_uses_status_causal_position() {
  ...
  set_mtime "$epoch" "$state/same-second.status"
  plant_task_outcome_index "$state" same-second 1 \
    || fail "could not plant the same-second presentation index"
...
test_drain_does_not_scan_append_only_outcome_history() {
  ...
  plant_task_outcome_index "$state" bounded-task 1 \
    || fail "could not plant the bounded-history presentation index"
  i=1
  while [ "$i" -le 20000 ]; do
