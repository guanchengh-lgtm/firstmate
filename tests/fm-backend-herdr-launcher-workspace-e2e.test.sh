...
for id in uniqA uniqB dupC dupD staleF smE presU presD; do
  ...
  printf '%s\n' 'Role: builder' 'trivial launcher-placement brief: nothing to do.' > "$PRIMARY_HOME/data/$id/brief.md"
  printf '%s\n' builder > "$PRIMARY_HOME/data/$id/role"
  printf '%s\n' no-mistakes > "$PRIMARY_HOME/data/$id/mode"
  printf '%s\n' 'Role: builder' 'trivial launcher-placement brief: nothing to do.' > "$SM_HOME/data/$id/brief.md"
  printf '%s\n' builder > "$SM_HOME/data/$id/role"
  printf '%s\n' no-mistakes > "$SM_HOME/data/$id/mode"
  printf '%s\n' 'Role: builder' 'trivial launcher-placement brief: nothing to do.' > "$PRES_HOME/data/$id/brief.md"
  printf '%s\n' builder > "$PRES_HOME/data/$id/role"
  printf '%s\n' no-mistakes > "$PRES_HOME/data/$id/mode"
done
...
spawn_from_launcher "" "$PRIMARY_HOME" uniqA "$PROJ" --mode no-mistakes --yolo off --role builder
...
spawn_from_launcher "$LAUNCH_PRIMARY_PANE" "$PRIMARY_HOME" uniqB "$PROJ" --mode no-mistakes --yolo off --role builder
...
spawn_from_launcher "$LAUNCH_PRIMARY_PANE" "$PRES_HOME" presU "$PROJ" --mode no-mistakes --yolo off --role builder
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/fm-spawn.sh" dupC "$PROJ" "sh -c 'echo launcher-ws-ok'" --mode no-mistakes --yolo off --role builder --backend herdr \\
...
spawn_from_launcher "$LAUNCH_DUP_PANE" "$PRES_HOME" presD "$PROJ" --mode no-mistakes --yolo off --role builder
...
spawn_from_launcher "" "$PRIMARY_HOME" dupD "$PROJ" --mode no-mistakes --yolo off --role builder
...
spawn_from_launcher "$STALE_PANE" "$PRIMARY_HOME" staleF "$PROJ" --mode no-mistakes --yolo off --role builder
...
spawn_from_launcher "$LAUNCH_SM_PANE" "$SM_HOME" smE "$PROJ" --mode no-mistakes --yolo off --role builder
...
printf 'miss:\nnumber:\npair:\npick:\nnone: test fixture\n' > "$PRIMARY_HOME/data/dupC/measure.md"
