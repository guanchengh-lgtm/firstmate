...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm1 "$PROJ1" "sh -c 'echo primary-crew-ok'" --mode no-mistakes --yolo off --role builder --backend herdr \
  >"$CM1_OUT" 2>"$CM1_ERR"
rc=$?
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$SM_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm2 "$PROJ2" "sh -c 'echo sm-crew-ok'" --mode no-mistakes --yolo off --role builder --backend herdr \
  >"$CM2_OUT" 2>"$CM2_ERR"
rc=$?
...
printf 'miss:\nnumber:\npair:\npick:\nnone: test fixture\n' > "$PRIMARY_HOME/data/cm1/measure.md"
