...
export async function createAgentSession(options) {
  ...
  const session = {
    ...
    get model() {
      return globalThis.__fmModel ?? options.model ?? {
        provider: "stub",
        id: "stub-model",
    ...
    getContextUsage() {
      const contextWindow = session.model.contextWindow ?? 500000;
...
test_branch_report_is_the_completion_handshake() {
  local repo home out status
  repo="$TMP_ROOT/report-handshake-root"
  home="$TMP_ROOT/report-handshake-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { dispatch, fire, defaultSessionCtx, mainUserMessages, sentToMain }; })()`);
const { dispatch, fire, defaultSessionCtx, mainUserMessages, sentToMain } = globalThis.__t;
...
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  ...
  const recorded = await report.execute(
    ...
    { task: "branch-driver", verdict: "routine", summary: "stored before the late provider error" },
    undefined,
    undefined,
    {},
  );
...
const offer = dispatch("signal: report before late provider error");
if (!offer.accepted) throw new Error("reported wake was not accepted");
await offer.settlement;
if (sentToMain.length !== 1) throw new Error("stored report was not delivered exactly once");
if (mainUserMessages.length !== 0) throw new Error("a completed report returned delivery ownership to main");
const next = dispatch("signal: report remains healthy");
if (!next.accepted) throw new Error("a late error after a completed report broke the branch");
await next.settlement;
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "a durable report must complete the wake even when the provider records a later error: $out"
  pass "a durable report is the branch wake completion handshake"
}

test_branch_context_budget_rotation() {
  local repo home out status
  repo="$TMP_ROOT/context-rotation-root"
  home="$TMP_ROOT/context-rotation-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
...
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { dispatch, fire, defaultSessionCtx, home }; })()`);
const { dispatch, fire, defaultSessionCtx, home } = globalThis.__t;
import { readFileSync, writeFileSync } from "node:fs";
...
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  ...
  const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");
  const recorded = await report.execute(
    `rotation-report-${reportNumber}`,
    { task: "branch-driver", verdict: "routine", summary: `rotation report ${reportNumber}` },
    undefined,
    undefined,
    {},
  );
  if (recorded.isError) throw new Error(`rotation report failed: ${JSON.stringify(recorded)}`);
};

globalThis.__fmContextTokens = 500;
await dispatch("signal: under budget").settlement;
const firstSession = globalThis.__fmSessions[0];
const firstFile = readFileSync(`${home}/state/.branch-session`, "utf8").trim();
writeFileSync(firstFile, "preserved session bytes\n");
const firstBytes = readFileSync(firstFile);
if (firstSession.disposed) throw new Error("under-budget branch rotated");

globalThis.__fmContextTokens = 1500;
await dispatch("signal: over budget").settlement;
if (!firstSession.disposed) throw new Error("over-budget branch did not rotate");
if (readFileSync(`${home}/state/.branch-session`, "utf8") !== "\n") {
  throw new Error("rotation did not clear the active session pointer");
}
if (!readFileSync(firstFile).equals(firstBytes)) throw new Error("rotation changed append-only session history");

globalThis.__fmContextTokens = 500;
await dispatch("signal: fresh branch after rotation").settlement;
const secondFile = readFileSync(`${home}/state/.branch-session`, "utf8").trim();
if (globalThis.__fmSessions.length !== 2 || secondFile === firstFile) {
  throw new Error("rotation did not create a fresh persistent branch session");
}
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "the branch must reuse an under-budget session and rotate an over-budget session without rewriting history: $out"
  pass "branch context budget rotates before exhaustion and preserves under-budget reuse"
}

test_branch_unreported_wake_breaker() {
  local repo home out status
  repo="$TMP_ROOT/unreported-breaker-root"
  home="$TMP_ROOT/unreported-breaker-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
...
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  attempt += 1;
  if (attempt !== 2 && attempt !== 6) return;
  const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");
  const recorded = await report.execute(
    `breaker-report-${attempt}`,
    { task: "branch-driver", verdict: "routine", summary: `breaker reset ${attempt}` },
    undefined,
    undefined,
    {},
  );
  if (recorded.isError) throw new Error(`breaker report failed: ${JSON.stringify(recorded)}`);
};
async function expectUnreported(message) {
  const offer = dispatch(message);
  if (!offer.accepted) throw new Error(`${message} was refused before the third strike`);
  const failure = await offer.settlement.then(() => null, (error) => error);
  if (!(failure instanceof Error) || !failure.message.includes("produced no durable outcome")) {
    throw new Error(`${message} did not fail as unreported: ${String(failure)}`);
  }
}

await expectUnreported("signal: pre-reset failure");
await dispatch("signal: report resets breaker").settlement;
for (let strike = 1; strike <= 3; strike += 1) {
  await expectUnreported(`signal: post-reset failure ${strike}`);
}
if (dispatch("signal: fourth wake must stay on main").accepted) {
  throw new Error("branch accepted a fourth wake after three consecutive unreported turns");
}
fire("session_shutdown", {});
fire("session_start", {}, defaultSessionCtx);
const replacement = dispatch("signal: replacement resets breaker");
if (!replacement.accepted) throw new Error("session replacement did not reset the breaker");
await replacement.settlement;
if (attempt !== 6) throw new Error(`branch prompted ${attempt} times instead of six`);
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "three consecutive unreported wakes must stop the branch until session replacement, while reports reset the streak: $out"
  pass "third consecutive unreported wake stops the branch loop"
}
...
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  ...
  if (attempt === 2 || attempt === 7 || attempt === 9) {
    const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");
...
const fifth = dispatch("signal: third consecutive provider error");
if (!fifth.accepted) throw new Error("the branch latched before the third consecutive provider error settled");
const fifthFailure = await fifth.settlement.then(() => null, (error) => error);
if (!(fifthFailure instanceof Error) || !fifthFailure.message.includes("provider failed after construction")) {
  throw new Error(`third consecutive provider error did not reject settlement: ${String(fifthFailure)}`);
}

const sixth = dispatch("signal: branch must now defer directly to main");
if (sixth.accepted) throw new Error("three consecutive provider errors did not latch the broken branch");
await new Promise((resolve) => setTimeout(resolve, 50));
if (attempt !== 5 || mainUserMessages.length !== 0) {
...
await settle(() => attempt === 6 && typeof releaseFailedProbe === "function", "in-flight failed cooldown probe");
...
await settle(() => attempt === 7 && sentToMain.some((sent) => sent.message.content.includes("cooldown probe recovered the branch")), "successful recovery probe");
...
await settle(() => attempt === 9 && sentToMain.some((sent) => sent.message.content.includes("post-recovery report proved")), "post-recovery healthy report");
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
...
// A reopened session retains the prior model, so clearing the pin needs an explicit main-model override.
...
const empty = scopeForUnreadWake(state, false);
if (empty.status !== "empty" || empty.eligible || empty.eligibleSeqs.length !== 0) {
  throw new Error(`an empty queue was not classified as empty: ${JSON.stringify(empty)}`);
}
...
for (const row of mainOnlyRows) {
  ...
  if (scope.status !== "main-only") {
    throw new Error(`an ordinary main-only row was misclassified: ${row} -> ${JSON.stringify(scope)}`);
  }
}
...
if (truncated.status !== "corrupted" || truncated.eligible || truncated.eligibleSeqs.length !== 0) {
...
if (heartbeatUnresolvable.eligible || heartbeatUnresolvable.status !== "corrupted") {
...
if (heartbeatUnknownKind.eligible || heartbeatUnknownKind.status !== "corrupted") {
...
if (heartbeatNothingOwnable.status !== "main-only") {
  throw new Error(`a purely main-only queue was misclassified: ${JSON.stringify(heartbeatNothingOwnable)}`);
}
process.exit(0);
EOF
  status=$?
