...
# A fake tmux that reports FM_FAKE_PANE_PATH after the leased-worktree `cd`
...
#     are disabled so the fm-<id> name survives entering the leased worktree;
#   - the exact worktree `cd` and the worktree settle loop target that stable
...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): direct lease acquisition precedes an exact cd on the stable id.
  assert_grep "treehouse get --lease --json --lease-holder rec-win-gg7" "$rec" \
    "treehouse must acquire a durable task-bound lease"
  grep -F "send-keys -t @spawnwid cd '" "$rec" | grep -F -- "$wt" >/dev/null \
    || fail "the exact leased path must be sent to the stable window id"
