...
SH
  ...
  command=$(brief_rewrite_emit "$command" "$parent/fakebin/record-emit")
...
# bash ${var/path/path} treats / as the delimiter, so a ROOT path never
# substitutes. Replace by literal index instead.
brief_rewrite_emit() {  # <command> <replacement-executable>
  printf '%s\n' "$1" | awk -v s="$ROOT/bin/fm-public-followup-emit.sh" -v d="$2" '
    BEGIN { sl = length(s) }
    {
      out = $0
      while (sl > 0 && (i = index(out, s)) > 0) {
        out = substr(out, 1, i - 1) d substr(out, i + sl)
      }
      print out
    }
  '
