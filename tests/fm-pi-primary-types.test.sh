...
command -v npm >/dev/null 2>&1 || { echo "skip: Pi extension typecheck unavailable - npm not found"; exit 0; }
command -v tsc >/dev/null 2>&1 || { echo "skip: Pi extension typecheck unavailable - tsc not found"; exit 0; }

PI_PACKAGE_DIR=${FM_PI_PACKAGE_DIR:-"$(npm root -g)/@earendil-works/pi-coding-agent"}
if [ ! -f "$PI_PACKAGE_DIR/package.json" ]; then
  echo "skip: Pi extension typecheck unavailable - installed @earendil-works/pi-coding-agent package not found"
  exit 0
fi
