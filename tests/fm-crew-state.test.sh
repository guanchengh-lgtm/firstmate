...
EOF
  ...
  assert_contains "$out" "state: working" "the executing row owns the branch even when its head is unresolvable"
  assert_contains "$out" "source: run-step" "the executing coarse row remains run-step sourced"
  pass "coarse scan binds the executing row and never falls to an older failed row"
}
...
test_non_pipeline_owned_unresolvable_head_not_attributed() {
  ...
  assert_contains "$out" "source: run-step" "an executing run binds even when branch_sync is not pipeline_owned"
  assert_contains "$out" "state: working" "the executing run still reads working"
  pass "an executing run binds without the pipeline-owned exemption"
}
