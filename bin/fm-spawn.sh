...
# delivery contract, so those axes must come from its durable record below.
...
HERDR_PROJECTION_ABORT_RECLAIMED_ACTIVE=0
...
spawn_abort_cleanup() {
  ...
  if [ "$HERDR_PROJECTION_ABORT_CLEANUP" = 1 ]; then
    HERDR_PROJECTION_ABORT_CLEANUP=0
    if [ "$HERDR_PROJECTION_ABORT_RECLAIMED_ACTIVE" = 1 ]; then
      HERDR_PROJECTION_ABORT_RECLAIMED_ACTIVE=0
...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    grok)
      ...
      # axis is the reasoning knob. As of grok 1.0.5, --reasoning-effort accepts
      # low|medium|high|xhigh and rejects max, so omit max rather than passing a
      # known-bad value.
      case "$effort" in
        ...
        if [ "${HERDR_RECOVERY_BACKEND:-}" = herdr ]; then
          ...
          case "$HERDR_RECLAIM_STATUS" in
            0)
              ...
              HERDR_PROJECTION_ABORT_SEEDED_PANE=""
              HERDR_PROJECTION_ABORT_RECLAIMED_ACTIVE=${FM_BACKEND_HERDR_PROJECTION_RECLAIMED_ACTIVE:-0}
