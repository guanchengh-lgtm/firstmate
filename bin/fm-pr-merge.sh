...
# A GitHub merge additionally requires readable no-mistakes validation truth,
# re-reads the forge rollup after recording, and passes that exact head to
# `gh pr merge --match-head-commit`; GitLab gets equivalent head immutability
# from its live pipeline-at-head check and `glab mr merge --sha`.
#
...
case "$PROVIDER" in
  github)
    ...
    gh pr merge "$PR_NUMBER" --repo "$PR_OWNER/$PR_REPO" \
      "${gh_args[@]+"${gh_args[@]}"}" --match-head-commit "$FM_VT_FORGE_HEAD"
    ;;
  gitlab)
