...
status_line_note() {  # <status-line> -> text after the first colon, trimmed
  ...
  # A note-head [key=...] token is key metadata, not note text. Strip a valid
  # one even when a before-colon key already won, so the leftover token cannot
  # remain in the folded note.
  if k=$(_fm_key_at_note_head "$1") && _fm_decision_slug_ok "$k"; then
    n=${n#"[key=$k]"}
    n=${n#"${n%%[![:space:]]*}"}
  fi
  printf '%s' "$n"
...
# Landed S1 fm-watch.sh still calls this name; keep it next to the working predicate.
