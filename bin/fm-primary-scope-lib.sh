...
fm_home_is_linked_worktree() {
  local home=$1 git_dir git_common_dir
  fm_root_is_secondmate_home "$home" && return 1
  [ "$(git -C "$home" rev-parse --is-inside-work-tree 2>/dev/null)" = true ] || return 1
  git_dir=$(git -C "$home" rev-parse --git-dir 2>/dev/null) || return 1
  git_common_dir=$(git -C "$home" rev-parse --git-common-dir 2>/dev/null) || return 1
...
fm_home_refuse_linked_worktree() {
  local home=$1 caller=$2 common real_home='<real-home>'
  fm_home_is_linked_worktree "$home" || return 0
...
# Return 1 for no match and 2 for an unverifiable ps/lsof result.
# On 2, stdout names the failed tool so command-substitution callers retain it.
fm_ancestor_cwd_in_linked_worktree() {
  local pid=$1 hops=0 info parent comm cwd output line
  ...
  while [ "$pid" -gt 1 ] && [ "$hops" -lt 64 ]; do
    ...
    [ -n "$cwd" ] || { printf 'lsof\n'; return 2; }
    if fm_home_is_linked_worktree "$cwd"; then
      printf '%s\t%s\t%s\n' "$pid" "$comm" "$cwd"
      return 0
    fi
    pid=$parent
