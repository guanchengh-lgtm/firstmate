...
# marked secondmate plain checkout - both operate a fleet and must dispatch
# through it - and rejects a linked task worktree, even when a leftover
# .fm-secondmate-home marker is present, which is the shape bin/fm-spawn.sh
# always hands a crewmate. A crewmate using delegation tools inside its own
# task worktree is legitimate and stays allowed. Any failure to confirm the
# home is inert (exit 0), never a block, so a broken environment never denies
# a call.
