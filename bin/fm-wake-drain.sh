...
# S4's landed processed-init writes the processed cursor only. Presentation
# indexes are a drain cache so S5 backstop can run before any later helper
# publishes the same files.
_drain_capture_status_position() {  # <task>
  local f="$STATE/$1.status" size ident size_after ident_after
  DRAIN_CAPTURED_STATUS_ENDPOINT=0
  DRAIN_CAPTURED_STATUS_IDENT=-
  ...
  case "$ident" in *$'\t'*|*$'\n'*|'') return 0 ;; esac
  DRAIN_CAPTURED_STATUS_ENDPOINT=$size
  DRAIN_CAPTURED_STATUS_IDENT=$ident
}

_drain_write_outcome_index() {  # <task> <seq> <endpoint> <ident>
  local task=$1 seq=$2 endpoint=$3 ident=$4 path tmp record
  case "$task" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  path="$STATE/.$task.branch-outcome-index"
  record=$(printf '%s\t%s\t%s\t%s\n' "$BRANCH_OUTCOME_INDEX_VERSION" "$seq" \
    "$endpoint" "$ident") || return 1
  [ "${#record}" -le "$BRANCH_OUTCOME_INDEX_MAX_BYTES" ] || return 1
...
_drain_publish_outcome_index_ready() {  # <seq>
  local tmp
  tmp=$(mktemp "$STATE/.branch-outcome-index-ready.XXXXXX") || return 1
  printf '%s\n' "$1" > "$tmp" || { rm -f -- "$tmp"; return 1; }
  mv -f -- "$tmp" "$STATE/.branch-outcome-index-ready"
}

_drain_outcome_store_last_seq() {  # <store>
  local store=$1
  [ -s "$store" ] || { printf '0\n'; return 0; }
  jq -Rse '
    def valid:
...
_drain_rebuild_outcome_indexes() {  # <store>; lock must already be held
  local store=$1 rows task seq epoch endpoint ident f mtime last
  last=$(_drain_outcome_store_last_seq "$store") || return 1
  rm -f -- "$STATE/.branch-outcome-index-ready" || return 1
  if [ ! -s "$store" ]; then
    _drain_publish_outcome_index_ready 0 || return 1
    return 0
  fi
  rows=$(jq -r -s '
        ...
        case "$mtime" in ''|*[!0-9]*) ;;
          *)
            if [ "$mtime" -lt "$epoch" ]; then
              _drain_capture_status_position "$task"
              endpoint=$DRAIN_CAPTURED_STATUS_ENDPOINT
              ident=$DRAIN_CAPTURED_STATUS_IDENT
            fi
            ;;
        esac
      fi
    fi
    _drain_write_outcome_index "$task" "$seq" "$endpoint" "$ident" || return 1
  done <<EOF
$rows
EOF
  _drain_publish_outcome_index_ready "$last"
}
...
print_status_outcome_backstop_section() {  # <task-and-endpoint-snapshot>
  ...
  if [ -e "$store" ] || [ -L "$store" ]; then
    ...
    if ! outcome_index_ready_ok "$ready"; then
      ...
      if ! outcome_index_ready_ok "$ready" && ! _drain_rebuild_outcome_indexes "$store"; then
