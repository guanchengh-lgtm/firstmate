...
if [ "${#REPLY_ENV[@]}" -gt 0 ]; then
  env ${REPLY_ENV[@]+"${REPLY_ENV[@]}"} "$FM_ROOT/bin/fm-x-reply.sh" "$RID" --followup ${TS_ARGS[@]+"${TS_ARGS[@]}"} >/dev/null
else
  "$FM_ROOT/bin/fm-x-reply.sh" "$RID" --followup ${TS_ARGS[@]+"${TS_ARGS[@]}"} >/dev/null
fi
post_rc=$?
