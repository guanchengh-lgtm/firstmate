...
#   complete <origin> (--none | <key>...) (--ideas <PI-id>... | --no-ideas)
#                                          -> complete <origin> with mapped ids and unchanged idea attestation
#   verify <origin>                        -> verify <origin>
#   state <hold-id> [--binding]            -> state <hold-id> [--binding]
#   supersede <origin> <key> --decision-file <f> --shipped-task <id>
#                                          -> supersede <origin>-decision-<key> with unchanged flags
...
command_complete() {
  local origin=${1:-} mapped='' idea_mode='' arg
  local -a idea_args=()
  [ "$#" -ge 2 ] || { usage >&2; exit 2; }
  ...
  while [ "$#" -gt 0 ]; do
    ...
    case "$arg" in
      --ideas)
        [ -z "$idea_mode" ] || fail "exactly one idea attestation is required"
        idea_mode=ideas
        idea_args=(--ideas)
        while [ "$#" -gt 0 ]; do
          idea_args+=("$1")
      ...
      --no-ideas)
        ...
        ;;
      --none)
        [ -z "$mapped" ] || fail "--none cannot be combined with decision keys"
        mapped=--none
        ;;
      *)
        [ "$mapped" != --none ] || fail "--none cannot be combined with decision keys"
        mapped="${mapped}${mapped:+ }$(compose "$origin" "$arg")"
        ;;
    esac
  done
  [ -n "$idea_mode" ] || fail "exactly one idea attestation is required (--ideas or --no-ideas)"
  # shellcheck disable=SC2086  # mapped is a validated space-separated slug list.
  exec "$CAPTAIN_HOLD" complete "$origin" $mapped "${idea_args[@]}"
}
...
command_supersede() {
  ...
  id=$(compose "$origin" "$key")
  shift 2
  exec "$CAPTAIN_HOLD" supersede "$id" "$@"
}
...
case "${1:-}" in
  ...
  state) shift; exec "$CAPTAIN_HOLD" state "$@" ;;
