...
fm_lock_prepare_owner() {
  ...
  fm_current_pid_into mypid || return 1
...
fm_lock_try_acquire() {
  ...
  # Resolve this frame's real pid before comparing: a Bash 3.2 subshell
  # inherits its parent's $$ and must never read as the parent holder.
  pid=$(cat "$lockdir/pid" 2>/dev/null || true)
  fm_current_pid_into current || return 1
...
fm_lock_acquire_wait_bounded() {
  ...
  fm_current_pid_into caller_pid || return 1
...
fm_autoarm_claim_next() {  # <state-dir> [grace]
  ...
  epoch="$state/.claude-autoarm-epoch"
  FM_AUTOARM_MY_GEN=
  # Resolve this frame's real pid before recording identity: a Bash 3.2
  # subshell inherits its parent's $$ and must never read as the parent holder.
  fm_current_pid_into pid || return 1
...
fm_autoarm_reset_owned() {  # <state-dir> <gen>
  ...
  lock="$state/.claude-autoarm.lock"
  fm_current_pid_into pid || return 2
